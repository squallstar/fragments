...
Meteor.methods({
  fragmentInsert: function (fragmentAttributes) {
    var user = Meteor.user() || fragmentAttributes.user;
    ...
    check(fragmentAttributes, {
      ...
      lead_image: Match.Optional(String),
      images: Match.Optional(Array),
      fetched_at: Match.Optional(Number),
    ...
    var fragment = _.extend(fragmentAttributes, {
      ...
      title: fragmentAttributes.title || (_.sample([
        'Fetching link',
        'Hang on',
        'Please wait',
        'Just one sec',
    ...
    if (Meteor.isServer && fragment.url && !fragment.fetched_at) {
      Meteor.defer(function () {
        Meteor.call('fragmentFetch', fragment._id);
      });
    }

    return fragment._id;
  },
  fragmentUndo: function (fragmentData) {
    this.unblock();

    check(Meteor.userId(), String);
    check(fragmentData, Object);
    check(fragmentData._id, String);

    return Fragments.insert(fragmentData);
  },
  ...
  fragmentArchive: function (fragmentId) {
    check(Meteor.userId(), String);
    check(fragmentId, String);
  ...
  fragmentTogglePin: function (fragmentId) {
    check(Meteor.userId(), String);
    check(fragmentId, String);
    ...
    Fragments.update(fragmentId, {
      $set: {
        pinned_at: fragment.pinned_at ? null : Math.round((new Date()).getTime() / 1000)
      }
    });
  },
