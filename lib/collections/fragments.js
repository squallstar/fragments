...
Meteor.methods({
  fragmentInsert: function (fragmentAttributes) {
    ...
    if (Meteor.isServer && fragment.url) {
      // Currently using sync call instead of queue to avoid UI delay
  ...
  fragmentDelete: function (fragmentId) {
    ...
    if (collections && collections.lenght) {
      canDelete = !!_.find(collections, (c) => {
        if (c.collaborators) {
          return _.find(c.collaborators, (u) => { return u._id === userId });
        }
      });
    } else {
      canDelete = fragment.user._id === Meteor.userId();
    }
